...
import { SpinningBox } from './spin-box'
...
export function Instances({ children, ...props }) {
  const { nodes } = useGLTF('/canvas/computers.glb')
...
export function Computers(props) {
  const { nodes: n, materials: m } = useGLTF('/canvas/computers.glb')
...
function Screen({ frame, panel, children, ...props }) {
  const { nodes, materials } = useGLTF('/canvas/computers.glb')
  return (
    <group {...props}>
...
function ScreenText({ invert, x = 0, y = 1.2, ...props }) {
  ...
  const color = invert ? 'Black' : 'Turquoise'
  const textColor = invert ? 'YellowGreen' : 'Black'
  ...
  return (
    <Screen {...props}>
      ...
      <color attach='background' args={[theme === 'dark' ? color : invert ? 'Turquoise' : 'Black']} />
